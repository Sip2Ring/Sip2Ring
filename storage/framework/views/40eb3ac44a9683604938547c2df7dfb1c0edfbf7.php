...
//                                $urls ="http://".$content['Name'].".s3-website".$urlConcatOperator.$locationConstraint.".amazonaws.com";
                               ...
                               /* if (preg_match('#[0-9]#',$finalString)){
                                ...
                                <tr>
                                    ...
                                    <td class="record_actions">
                                        ...
                                        <a onclick="return checkAid('<?php echo e('#analyticsRecordId_'.$recordCounter); ?>');" title="Analytics" class="btn btn-danger1" data-bucket="<?php echo e($content['Name']); ?>" id="<?php echo e('analyticsRecordId_'.$recordCounter); ?>" data-location="<?php echo e($locationConstraint); ?>" data-id="<?php echo e($awsId); ?>" data-anLink="<?php echo e($urls.'/assests/analytics.js'); ?>" > <i class="fa fa-google fa-lg"></i>
                                        </a>
                                    </td>
                                </tr>
                                <?php }
    ...
    <div id="analytics_dialogue" class="modal fade form-group " role="dialog">
        <div class="modal-dialog">
            <form name="analytics_form" id="analytics_form"  method="post">
                <input type="hidden" id="dpass_token_aws" name="_token" value="<?php echo e(csrf_token()); ?>">
                <input type="hidden" name="analytics_aws_id" id="analytics_aws_id" value = "<?php echo e($awsId); ?>">
                <input type="hidden" id="template_name_aid" name="template_name" value="">
                <input type="hidden" id="region_aid" name="region" value="">
                <!-- Modal content-->
                <div class="modal-content">
                    <div class="modal-header">
                        <h2>Analytics Id:</h2>
    ...
    <script>
        ...
        $(document).on('click','#analytics_id_update',function(){
            var bucket_name                 = $('#template_name_aid').val();
            var region                      = $('#region_aid').val();
            var file_path                   = $('#analytics_url').val();
            var analytics_id                = $('#analytics_id').val();
            var analytics_aws_id            = $('#analytics_aws_id').val();
            var passToken                   = $('#dpass_token_aws').val();
            var searchItem                  = $('div.dataTables_filter input').val();
            var url                         = "<?php echo e(url('/update-analytics-fie')); ?>";
            var successRedirect 			= "<?php echo e(url('/buckets')); ?>?x="+searchItem;
            $.ajax({
                type: 'POST',
                'url': url,
                ...
                data: {
                    '_token': passToken,
                    'analytics_bucket_name': bucket_name,
                    'analytics_bucket_region': region,
                    'analytics_aws_id': analytics_aws_id,
        ...
        function checkPhone(recordID) {
			...
			/* check analytics */
			 function checkAid(analyticsRecordId) {
            var ajax_url        = "<?php echo e(url('/check-analytics-url')); ?>";
            var passToken       = '<?php echo e(csrf_token()); ?>';
            var analyticsLink   = $(analyticsRecordId).attr('data-anLink');
            var bucketName      = $(analyticsRecordId).attr('data-bucket');
            var bucketRegion    = $(analyticsRecordId).attr('data-location');
            $.ajax({
                type: 'POST',
                'url': ajax_url,
                async: false,
                ...
                success:function(data){
                    ...
                    var res = jQuery.parseJSON(data);
                    if(res.type=='success'){
         			    $('#analytics_id').val(res.aId);
                        $('#template_name_aid').val(bucketName);
                        $('#region_aid').val(bucketRegion);
						
                       $('#analytics_dialogue').modal('show');
                        return false;
                    }
                    if(res.type=='error'){
                        alert(res.message);
